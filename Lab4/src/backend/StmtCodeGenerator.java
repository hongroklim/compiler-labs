...
public class StmtCodeGenerator extends Visitor<Void> {
    ...
    Unit exit = breakTargets.get(nd.getEnclosingLoop());
    units.add(j.newGotoStmt(exit));
		return null;
	}
	...
	public Void visitWhileStmt(WhileStmt nd) {
		 ...
		 */
    NopStmt enter = j.newNopStmt();
    NopStmt exit = j.newNopStmt();

    breakTargets.put(nd, exit);
